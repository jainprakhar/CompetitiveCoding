...
void match1()
{
	...
	player* i3 = getPlayer("Bumrah",1,0,0,2,1,0, 0, 0, 2,20,0);
	...
	{
		if(!all[i]->name.compare(manMatch->name))
		{
			all[i]->rating += 10; 
		}
	}
...
int main()
{
	...
	cout << "Name 	" << "Rating 	" << endl;
 	for(int i=0;i<all.size();i++)
	{
		cout << all[i]->name << "	" << all[i]->rating << endl;
	}
